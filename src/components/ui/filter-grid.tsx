        ...
        {filters.map((filter, i) => {
          ...
          return (
            <button
              ...
              className="group relative inline-grid h-9 select-none place-items-center rounded-full px-4 outline-none focus-visible:shadow-[0_0_0_2px_#07559e,0_0_0_4px_rgba(255,219,0,0.6)]"
            ...
            >
              {on ? (
                <motion.span
                  ...
                  transition={CELL}
                  className="absolute inset-0 rounded-full bg-[#ffdb00]"
                />
              ) : null}

              <span
                aria-hidden
                className={`pointer-events-none absolute inset-0 rounded-full border transition-colors group-focus-visible:border-[#ffdb00] ${
                  on ? 'border-transparent' : 'border-white/35 group-hover:border-white'
                }`}
              />
              <span className="relative col-start-1 row-start-1 inline-grid">
                <motion.span
                  ...
                  transition={swap}
                  className="col-start-1 row-start-1 inline-flex items-center gap-1.5 whitespace-nowrap text-sm font-semibold text-white/85"
                >
                  {filter.label}
                  <span className="text-xs tabular-nums text-white/60">{counts[filter.id]}</span>
                </motion.span>
                <motion.span
                  aria-hidden
                  initial={false}
                  ...
                  transition={swap}
                  className="col-start-1 row-start-1 inline-flex items-center gap-1.5 whitespace-nowrap text-sm font-semibold text-[#16003C]"
                >
                  {filter.label}
                  <span className="text-xs tabular-nums opacity-70">{counts[filter.id]}</span>
                </motion.span>
                <span className="sr-only">
                  {filter.label}, {counts[filter.id]} de {total}
                </span>
              </span>
            </button>
          )
      ...
      <div className="relative mt-3">
          ...
          <AnimatePresence initial={false} mode="popLayout" onExitComplete={settle}>
            {visible.map((item) => (
              <motion.li
                ...
                transition={step}
                className="min-w-0 overflow-hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_1px_2px_rgba(14,27,60,0.06),0_4px_10px_-8px_rgba(14,27,60,0.45)]"
              >
                {renderItem(item)}
              </motion.li>
            ))}
        ...
        <AnimatePresence initial={false}>
          {visible.length === 0 && (
            ...
            >
              <span className="px-4 text-center text-sm text-white/85">{emptyLabel}</span>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      <p aria-live="polite" className="sr-only">
        {activeLabel}: {visible.length} de {total} mostrados
      </p>
    </div>
  )
}
